...
const Register = () =>
{
    const { formControl,getEmail,getPassword,registerWithPassword } = useAPI().auth;
    return (
        <div>
            <form onSubmit={formControl} className='w-2/4 text-center shadow border p-6 rounded m-auto my-20'>
                ...
                <input onClick={registerWithPassword} className='p-3 cursor-pointer block w-full bg-orange text-lg font-primary text-white rounded' value='Login' type="submit" />
            </form>
        </div>
    );
};
